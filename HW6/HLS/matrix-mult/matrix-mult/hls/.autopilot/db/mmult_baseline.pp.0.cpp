...
# 11 "./mmult_baseline.h" 2



__attribute__((sdx_kernel("mmult_hw", 0))) void mmult_hw(int in1[128 * 128],
              int in2[128 * 128],
              int out[128 * 128],
              int dim);
# 6 "mmult_baseline.cpp" 2
# 1 "C:/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot\\ap_int.h" 1
# 7 "mmult_baseline.cpp" 2
...
#pragma HLS ARRAY_PARTITION variable=B cyclic factor=8 dim=1
 ...
 VITIS_LOOP_29_1: for (int i = 0; i < 128; i++) {
        VITIS_LOOP_30_2: for (int j = 0; j < 128; j++) {
#pragma HLS PIPELINE II=1
 A[i][j] = in1[i * 128 + j];
        }
    }

    VITIS_LOOP_36_3: for (int i = 0; i < 128; i++) {
        VITIS_LOOP_37_4: for (int j = 0; j < 128; j++) {
#pragma HLS PIPELINE II=1
 B[i][j] = in2[i * 128 + j];
        }
    }

    VITIS_LOOP_43_5: for (int i = 0; i < 128; i++) {
        VITIS_LOOP_44_6: for (int j = 0; j < 128; j++) {
#pragma HLS PIPELINE II=1
 int sum = 0;
            int sum_array[8] = {0};

            VITIS_LOOP_49_7: for (int k = 0; k < 128; k += 8) {
                VITIS_LOOP_50_8: for (int u = 0; u < 8; u++) {
#pragma HLS UNROLL factor=8
 sum_array[u] += A[i][k + u] * B[k + u][j];
                }
            }

            VITIS_LOOP_56_9: for (int u = 0; u < 8; u++) sum += sum_array[u];
            out[i * 128 + j] = sum;
        }
    }
}
